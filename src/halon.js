...
function expandLink( action, data ) {
	let href = URI.expand( action.href, data ).href();
	const query = data[ "?" ];
	if ( query ) {
...
function processResponse( response, fsm ) {
	...
	if ( !response._links ) {
		const listKey = Object.keys( response )
			.filter( function( x ) {
				return x !== "_origin";
			} )[ 0 ];
		const base = { _origin: response._origin };
...
function processKnownOptions( knownOptions ) {
	const x = _.reduce( knownOptions, function( memo, rels, resource ) {
	...
	states: {
		...
		ready: {
			...
			"invoke.resource": function( resource, rel, data, headers, success, err ) {
				let resourceDef = resource._links[ rel ];
				if ( !resourceDef ) {
...
const halonFactory = function( options ) {
	const defaults = {
		version: 1,
		knownOptions: {},
	...
	const client = function halonInstance() {
		const args = Array.prototype.slice.call( arguments, 0 );
		return when.all( args );
	};
	options = _.defaults( options, defaults );
	const server = /http(s)?[:][\/]{2}[^\/]*/.exec( options.root );
	options.server = server ? server[ 0 ] : undefined;
	options.client = client;

	const fsm = client.fsm = new HalonClientFsm( options );
	...
	client.connect = function() {
		const pending = this.deferred && this.deferred.promise.inspect().state !== "fulfilled";
		if ( !pending ) {
...
halonFactory.jQueryAdapter = function( $ ) {
	const autoStringify = [ "PATCH", "POST", "PUT" ];
...
halonFactory.requestAdapter = function( request ) {
	return function( link, options ) {
		...
		const json = _.isString( options.data ) ?
			JSON.parse( options.data ) :
			options.data;
		const url = link.href.indexOf( options.server ) < 0 ?
